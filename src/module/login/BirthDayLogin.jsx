...
export default function BirthdayLogin() {
    ...
    const handleLogin = async () => {
        ...
        const isValidEmployeeCode = /^MB\d{5}$/.test(code);
        if (!isAdmin && !isValidEmployeeCode) {
            return toast.error('Mã nhân viên không hợp lệ! Phải bắt đầu bằng "MB" và 5 chữ số cuối. Nếu mã nhân viên có 4 số thì thêm 0 ở trước. VD: "MB9999" => "MB09999"');
        }
        const info = {...infos}
